...
function keyCharPressed(e) {
   ...
   // var keychar = addEventListener('compositionstart', getPressedChar);
    ...
    if (keychar === currentChar || ((currentChar === '\n' || currentChar === '\r\n' ||
        ...
        if (/\s/.test(keychar)) {
            // Whitespace boundaries are handled by current position tracking.
        }
...
function keyCompPressed(e) {
    ...
    if (newInput === "") { // Backspace?
        prevInput = inputStr;
        return false;
    } else if (newInput === currentChar) {
        // Works for one output character (for example Amharic). Multi-output character
        // paths (for example Japanese "cha" -> "ã¡ã‚ƒ") use sequence checks below.
 ...
 */
// Called from inline JS in view.php when the typing page loads.
// eslint-disable-next-line no-unused-vars
